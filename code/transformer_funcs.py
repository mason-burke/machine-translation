...
class Atten_Head(tf.keras.layers.Layer):
	def __init__(self, input_size, output_size, use_mask):
		...
		self.K = self.add_weight(shape = [input_size, output_size], trainable = True)
		self.V = self.add_weight(shape = [input_size, output_size], trainable = True)
		self.Q = self.add_weight(shape = [input_size, output_size], trainable = True)
	...
	def call(self, inputs_for_keys, inputs_for_values, inputs_for_queries):
		...
		out = Attention_Matrix(K, Q, self.use_mask)

		return tf.matmul(out, V)
